              ...
              <template v-slot:default>
                <thead>
                  <tr style="border-bottom: thin solid rgba(0, 0, 0, 0.12)" class="">
                    ...
                    <th class="text-left th_color">Verification Status</th>
                ...
                <tbody>
                  <tr v-for="user in users" :key="user.index" class="mt-2"
                    ...
                    <!-- <td>{{ user.email }}</td> -->
                    <td>{{user.verification_status.toLowerCase()}}</td>
                  </tr>
...
export default {
  ...
  methods: {
    async getBankVerifications() {
      try {
        let response = await this.$axios.get("/admin/getAllVerification");
