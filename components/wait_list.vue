            ...
            <v-simple-table fixed-header height="100%">
              <template v-slot:default>
                ...
                <tbody>
                  ...
                  <tr v-if="wait_list.length == 0">
                    <td>
                      <p class="text-center">
                        You have no waitlist.
...
export default {
  ...
  methods: {
    async getWaitList() {
      try {
        let response = await this.$axios.get("/admin/getWaitList");
        this.wait_list = response.data;
        console.log(this.wait_list);
      } catch (error) {
